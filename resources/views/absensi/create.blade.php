...
// === QR SCANNER PERTAMA DITAMPILKAN ===
        ...
        html5QrCode.start(
            ...
            qrCodeMessage => {
                html5QrCode.stop().then(() => {
                    ...
                    // 4. Start webcam kamera depan
                    Webcam.set({
                        width: window.innerWidth * 0.9,
                        height: window.innerHeight * 0.4,
                        image_format: 'jpeg',
                        jpeg_quality: 80,
                        constraints: { facingMode: "user" } // kamera depan
                    });
                    ...
                    // 5. Ambil lokasi dan tampilkan peta
                    if (navigator.geolocation) {
...
$("#presensi").click(function () {
    ...
    $.ajax({
        ...
        success: function (respond) {
            const status = respond.split("|").map(item => item.trim());
...
function showMap(posisi) {
    ...
    lokasiInput.value = posisi.coords.latitude + "," + posisi.coords.longitude;

    const map = L.map('map').setView([posisi.coords.latitude, posisi.coords.longitude], 18);
    const lokasi_sekolah = "{{ $lok_sekolah->lokasi_sekolah }}";
    const [lat_sekolah, long_sekolah] = lokasi_sekolah.split(",");
    const radius = "{{ $lok_sekolah->radius }}";
